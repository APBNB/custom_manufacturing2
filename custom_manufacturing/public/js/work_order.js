...
frappe.ui.form.handlers["Work Order"] = {};

if (window.cur_frm && cur_frm.doctype === "Work Order") {
	cur_frm.events = {};
}
...
frappe.ui.form.on("Work Order", {
	...
	setTimeout(() => {
		const $table = $(dialog.$wrapper).find(".grid-heading-row, .grid-row");
		$table.css({
			display: "grid",
			gridTemplateColumns:
		...
		// Make header labels bold and center-aligned
		$(dialog.$wrapper)
			.find(".grid-heading-row > div")
			.css({ "font-weight": "600", "text-align": "center" });
	}, 300);
	// --- end fix ---
	...
	frm.doc.operations.forEach((data) => {
		if (data.completed_qty + data.process_loss_qty != frm.doc.qty) {
			...
			if (pending_qty || pending_qty === 0) {
				dialog.fields_dict.operations.df.data.push({
					name: data.name,
					operation: data.operation,
					custom_shift_number: data.custom_shift_number || null,
...
erpnext.work_order = {
	...
	show_prompt_for_qty_input: function (frm, purpose) {
		...
		const open_prompt = (total_from_job_cards = 0) => {
			const qty_precision = precision("qty");
			const job_card_total = flt(total_from_job_cards || 0, qty_precision);
			let default_qty = flt(max, qty_precision);
			let description = __("Max: {0}", [default_qty]);

			if (purpose === "Manufacture" && job_card_total) {
				description = __("Max: {0}. Total from Job Cards: {1}", [
					default_qty,
					job_card_total,
				]);

				if (max) {
					default_qty = flt(Math.min(job_card_total, max), qty_precision);
				} else {
					default_qty = job_card_total;
				}
			}

			const base_qty = default_qty;
			...
			if (purpose === "Manufacture") {
				fields.push({
					fieldtype: "Float",
					label: __("Total from Job Cards"),
					fieldname: "job_card_total_qty",
					read_only: 1,
					default: job_card_total,
					precision: qty_precision,
				});
			}
			...
			fields.push({
				...
				default: 0,
				onchange: function () {
					const process_loss = flt(frm.doc.process_loss_qty || 0, qty_precision);
					if (this.value) {
						frm.qty_prompt.set_value("qty", max - process_loss);
					} else {
						frm.qty_prompt.set_value("qty", base_qty);
					}
				},
			});
			...
			return new Promise((resolve, reject) => {
				let overproduction_percentage = 0;
				if (frm.doc.__onload && frm.doc.__onload.overproduction_percentage) {
					overproduction_percentage = frm.doc.__onload.overproduction_percentage;
				}

				const dialog = frappe.prompt(
					fields,
					(data) => {
						const limit = max + (frm.doc.qty * overproduction_percentage) / 100;

						if (data.qty > limit) {
							frappe.msgprint(__("Quantity must not be more than {0}", [limit]));
							reject();
						}
				...
				dialog.set_value("qty", base_qty);

				if (purpose === "Manufacture") {
					dialog.set_value("job_card_total_qty", job_card_total);
				}
			});
		};
		...
		if (purpose === "Manufacture") {
			return frappe
				.xcall("custom_manufacturing.api.work_order.get_total_manufactured_qty", {
					work_order: frm.doc.name,
				})
				.then((total) => open_prompt(total))
				.catch(() => open_prompt());
		}
