...
frappe.ui.form.on("Material Request", {
			...
			if (flt(frm.doc.per_ordered, precision) < 100) {
				...
				if (frm.doc.material_request_type === "Material Transfer") {
					frm.add_custom_button(
                        ...
                        () => frm.events.make_material_transfer_for_manufacture(frm),
                        __("Create")
                    );
				}
    ...
    //function kinked to custom bbutton Material Transfer for Manufacture
    make_material_transfer_for_manufacture(frm) {
    frappe.call({
        method: "custom_manufacturing.doc_events.make_material_transfer_for_manufacture.make_material_transfer_for_manufacture",
        args: {
            material_request: frm.doc.name,
        },
        freeze: true,
